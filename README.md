...
CSV 読込：東証データやユーザー指定 CSV から銘柄情報を抽出

構成比表示：業種別・銘柄別の保有比率をグラフで可視化

損益・配当トレンド：損益推移と配当実績を時系列で表示

指標取得：PER/PBR/EPS を外部 API またはローカル DB から自動取得

配当予測：想定年間配当額と配当利回りを算出

株価チャート：Matplotlib による銘柄別株価推移グラフ生成
...
Java GUI から CSV を選択すると、Python スクリプトがバックエンドで実行されます

分析結果は GUI 上に表示され、グラフは output/ フォルダに保存されます
